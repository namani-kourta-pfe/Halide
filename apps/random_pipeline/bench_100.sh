...
mkdir bin
cp ../../bin/libautoschedule_adams2019.so bin/
mv bin/libautoschedule_adams2019.so bin/libauto_schedule.so

# Build the shared things by building one pipeline
HL_TARGET=x86-64-linux HL_SEED=root PIPELINE_SEED=0 HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 make build
...
for ((b=0;b<10000000;b++)); do
	...
	echo "HL_TARGET=x86-64-linux HL_SEED=root PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 make build 2>&1 | grep -v Nothing.to.be.done"
	for ((s=0;s<$SCHEDULES;s++)); do
            echo "HL_TARGET=x86-64-linux HL_SEED=$s PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 make build 2>&1 | grep -v Nothing.to.be.done"
	done
	...
	if [ ! -f $F ]; then HL_TARGET=x86-64-linux HL_SEED=root PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 HL_NUM_THREADS=24 numactl --cpunodebind=1 make bench 2>&1 | grep -v "Nothing to be done"; fi
	...
	for ((s=0;s<$SCHEDULES;s++)); do
	    F=bin/host-new_autoscheduler/pipeline_${P}_${STAGES}/schedule_${s}_50_1/times.txt
            if [ ! -f $F ]; then HL_TARGET=x86-64-linux HL_SEED=$s PIPELINE_SEED=$P PIPELINE_STAGES=$STAGES HL_RANDOM_DROPOUT=50 HL_BEAM_SIZE=1 HL_NUM_THREADS=24 numactl --cpunodebind=1 make bench 2>&1 | grep -v "Nothing to be done"; fi
